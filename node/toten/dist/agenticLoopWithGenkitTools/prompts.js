...
`;
function buildPlanPrompt(state, availableToolsText) {
    return `
        GOAL: ${state.goal}
...
function buildActPrompt(state, instruction) {
    return `
        GOAL: ${state.goal}
...
function buildCriticPrompt(state, actOutput) {
    return `
        GOAL: ${state.goal}

        HISTORY: ${state.history
        .map((h) => `Iteration ${h.iteration}: plan="${h.planInstruction}", actOutput="${h.actOutput}", criticObservations="${h.criticObservations || ""}"`)
        .join("\n") || "<none>"}
